...
@implementation UIImage (EIPUploadImage)
+ (UIImage *)eipCompressImageQuality:(UIImage *)image toByte:(NSInteger)maxLength {
    NSData *data1 = UIImageJPEGRepresentation(image, 1);
    
    NSLog(@"data.length=====%lu",(unsigned long)data1.length);
    
    NSLog(@"image的图片格式是===%@",[self eipImageFormat:image]);
...
/**
 ...
 @return 返回图片格式
 */
+ (NSString *)eipImageFormat:(UIImage *)image{
